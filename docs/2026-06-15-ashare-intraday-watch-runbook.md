...
- 全 A 宽度: 目前是采集清单宽度,无法代表全市场上涨/下跌家数、涨停/跌停家数、炸板率。
- 涨停梯队/连板结构: 目前没有稳定涨停状态源,无法识别一进二、二进三、空间板、断板潮。
- 资金流驱动的题材确认: 已有部分资金流基础,但实盘消息状态机尚未把主力净流入、板块资金流和个股资金流纳入核心规则。
- 题材内部强弱排名连续性: 当前有快照和状态,但还没有“连续 N 个 5m 上升/下降”的趋势确认。
- 题材间轮动: 当前能看单题材状态,还不能系统识别“AI 转弱、机器人接力”这类跨题材轮动。
...
1. 接入真正的全 A 宽度或低成本宽度代理,替换“采集清单宽度”的局限。
2. 在题材状态机中加入资金流确认,减少纯涨跌幅噪声。
3. 增加涨停/连板/炸板结构识别,这是 A 股短线盘中判断的重要缺口。
4. 做盘后回放页面:按时间轴复盘 `live_messages`、`theme_states`、指数和采集宽度。
5. 开盘实盘复测阈值,记录误报/漏报,再调 `RULE_VERSION`。
