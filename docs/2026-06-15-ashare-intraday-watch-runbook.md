...
- `题材核心资金流确认`: 题材处于启动/扩散,且当天已入库核心股主力净流入合计为正。
- `题材上涨但资金流背离`: 题材处于启动/扩散,但当天已入库核心股主力净流入合计为负。
- `题材涨停结构增强`: 采集清单内出现涨停/近涨停结构代理,用于弥补暂未接入全 A 涨停池的缺口。
- `题材连板结构增强`: collector 进程读取本地 DuckDB 昨日已收线日线,发现昨日强势股今日继续接近涨停。
- `题材炸板风险出现`: 盘中 quote 峰值曾接近涨停,随后明显回落到阈值以下,用于代理开板/炸板风险。
...
- `ThemeState.evidence` 追加成交额覆盖、领涨成交占比、近涨停数量、强势股数量、昨日强势延续和盘中高位回落等结构字段;这仍是采集清单内代理,不是全市场涨停/炸板统计。
- `ThemeState.evidence` 追加当天核心股资金流证据;只读取 BJT 当天已入库数据,旧资金流不会参与判断。当日缺失时标记 `fund_flow_available=false`,不生成资金流确认/背离消息。
...
- 涨停梯队/连板结构: 已有采集清单内昨日强势延续、近涨停、炸板和龙头回落代理,但没有稳定涨停池/盘口源,仍无法识别全市场一进二、二进三、空间板、断板潮和真实炸板率。
- 资金流驱动的题材确认: 状态机已纳入当天已入库核心股资金流,但 A 股 collector 暂未恢复大规模盘中资金流采集,板块资金流表仍为空。
...
- 已补题材核心股资金流证据接入: 当天已入库数据可触发资金流确认/背离,旧数据安全忽略。
...
2. 设计低 QPS 的 A 股核心股资金流采集策略,扩大资金流确认覆盖面;板块资金流仍需稳定数据源。
