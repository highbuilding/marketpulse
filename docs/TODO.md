...
- [x] **全 A 宽度或低成本宽度代理** _(2026-06-16)_
  - 已做:保留“采集清单宽度”作为低成本代理,不恢复盘中 EM 全 A 轮询;新增 5 分钟桶基准下的 `采集样本宽度快速恶化/改善`、样本涨停结构和跌停风险代理。
  - 边界:这仍不是全 A 真实宽度,消息文案和 payload 均标注 `sample_scope=当前采集清单,非全A`。
  - 后续:如要接真全 A,需先解决东财全 A 快照盘中超时/QPS 问题,或找到更稳定低频数据源。

- [x] **题材状态机加入资金流确认** _(2026-06-16)_
  - 已做:采集清单内成交额结构已写入 `ThemeState.evidence`;状态机现在会读取当天已入库的核心股主力净流入,生成 `核心资金流确认` / `上涨但资金流背离`。
  - 安全边界:只使用 BJT 当天已入库资金流,旧数据不会参与;如果当日核心股资金流缺失,只写 `fund_flow_available=false`,不生成资金流判断。
  - 后续:当前 A 股 collector 仍未恢复大规模 `stock_individual_fund_flow` 盘中拉取,板块资金流表也为空。若要扩大覆盖,需要先设计低 QPS 的核心股资金流采集策略。

- [x] **涨停/连板/炸板结构识别** _(2026-06-16)_
  - 已做:采集清单内代理已覆盖 `涨停结构增强`、`连板结构增强`、`炸板风险出现`、`龙头高位回落`。
  - 数据口径:连板代理读取 collector 进程本地 DuckDB 中昨日已收线日线,判断昨日强势股今日是否继续接近涨停;炸板代理用盘中 quote 峰值判断曾接近涨停后明显回落。
  - 边界:这仍不是全 A 涨停池/真实封单/开板次数统计,无法给出全市场连板高度、空间板、炸板率。要做真实版本需要稳定涨停池或盘口源。

- [x] **题材间轮动识别** _(2026-06-16)_
  - 已做:新题材进入启动/扩散,且近 30 分钟内有旧题材转弱/回落时,生成 `题材轮动` live message。
  - 后续:实盘复测阈值,再考虑加入资金流和题材强弱连续性。

- [x] **盘后回放视图** _(2026-06-16)_
  - 已做:`/api/replay` + `/replay` 页面,按交易日串起 `live_messages`、`theme_snapshots` 和核心指数 5m 背景。
  - 后续:补更多盘后统计,例如误报标记、消息重要性回放、题材轮动泳道。
