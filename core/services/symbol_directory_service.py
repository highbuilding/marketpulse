...
from core.services._crypto_seeds import CRYPTO_SEEDS
...
_INDEX_SEEDS: list[tuple[str, str, str]] = [
    ...
    ("000905.SH", "中证500", "ashare"),
    ("000852.SH", "中证1000", "ashare"),
    ("000688.SH", "科创50", "ashare"),
    ("000016.SH", "上证50", "ashare"),
    ("HSI.HK", "恒生指数", "hk"),
    ("HSTECH.HK", "恒生科技指数", "hk"),
...
class SymbolDirectoryService:
    ...
    async def bootstrap_seeds(self) -> None:
        """写入指数 + crypto 静态种子(快,启动时同步跑)。纯本地写库,无外部调用。"""
        await self.repo.upsert_many(_INDEX_SEEDS + CRYPTO_SEEDS)
