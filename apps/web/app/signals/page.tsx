...
import { DEMO_SIGNALS, type SignalDirection } from '@/lib/demo_fixtures'
import type { MarketId } from '@/lib/market-context'
...
const INTERVAL_OPTS = ['全部', '1d', '4h', '60m', '30m', '15m', '1h']

export default function SignalsPage() {
  const router = useRouter()
  const [market, setMarket] = useState<MarketFilter>('all')
  const [dirs, setDirs] = useState<SignalDirection[]>(['buy', 'sell'])
  const [interval, setIntervalSel] = useState('全部')

  const toggleDir = (d: SignalDirection) =>
    setDirs((cur) => (cur.includes(d) ? cur.filter((x) => x !== d) : [...cur, d]))

  const rows = DEMO_SIGNALS.filter((s) =>
    (market === 'all' || s.market === market) &&
    dirs.includes(s.direction) &&
  ...
  return (
    <div style={{ padding: 24, maxWidth: 920, margin: '0 auto' }}>
      ...
      <div className="panel">
        ...
        {rows.map((s) => (
          <div key={s.id} onClick={() => router.push(`/symbol/${s.symbol}`)}
            ...
            <span className={`sig-badge ${s.direction}`} style={{ width: 32, height: 32, fontSize: 14 }}>
              {s.direction === 'buy' ? '▲' : '▼'}
            </span>
            <span style={{ color: s.direction === 'buy' ? 'var(--red)' : 'var(--green)', fontSize: 12, fontWeight: 600, width: 28 }}>
              {s.direction === 'buy' ? '买入' : '卖出'}
            </span>
            <b style={{ minWidth: 84 }}>{s.name}</b>
            <span className="font-mono sig-mkt">{s.symbol}</span>
            <span style={{ color: 'var(--text3)', fontSize: 12 }}>{s.interval}</span>
            <span style={{ color: 'var(--text2)', fontSize: 12 }}>{s.type}</span>
            <span className="font-mono" style={{ marginLeft: 'auto', color: 'var(--text3)', fontSize: 12 }}>{s.ts}</span>
          </div>
        ))}
      </div>
    </div>
